...
public:
	...
	reward slide(unsigned opcode, int& maxPlace) {
		last_slide_side = opcode;
		switch (opcode & 0b11) {
		case 0: return slide_up(maxPlace);
		case 1: return slide_right(maxPlace);
		case 2: return slide_down(maxPlace);
		case 3: return slide_left(maxPlace);
		default: return -1;
		}
	}

	reward slide_left(int& maxPlace) {
		board prev = *this;
	...
	reward slide_right(int& maxPlace) {
		reflect_horizontal();
		reward score = slide_left(maxPlace);
		reflect_horizontal();
		return score;
	}
	reward slide_up(int& maxPlace) {
		rotate_right();
		reward score = slide_right(maxPlace);
		rotate_left();
		return score;
	}
	reward slide_down(int& maxPlace) {
		rotate_right();
		reward score = slide_left(maxPlace);
		rotate_left();
		return score;
